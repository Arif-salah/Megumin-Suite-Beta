...
export function getParsedBlockTypes(mesText) {
    ...
    // These two are non-global, so .test() has no position to remember.
    if (BLOCK_PATTERNS.worldState.test(mesText)) found.add("worldState");
    if (BLOCK_PATTERNS.innerChatter.test(mesText)) found.add("innerChatter");
    ...
    // non-global copy instead.
    if (new RegExp(NEW_NPC_PATTERN.source, "iu").test(mesText)) found.add("newNpc");
    return found;
}
...
export function getParsedBlockCounts(mesText) {
    ...
    if (BLOCK_PATTERNS.worldState.test(mesText)) counts.worldState = 1;
    if (BLOCK_PATTERNS.innerChatter.test(mesText)) counts.innerChatter = 1;
    ...
    const scan = new RegExp(NEW_NPC_PATTERN.source, "igu");
    while (scan.exec(mesText) !== null) counts.newNpc++;
    return counts;
}
